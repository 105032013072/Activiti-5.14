...
import org.activiti.engine.impl.cmd.AddBizCommentsCmd;
import org.activiti.engine.impl.cmd.DeleteBizCommentCmd;
...
import org.activiti.engine.impl.cmd.GetAfterActivtiyCmd;
import org.activiti.engine.impl.cmd.GetBizCommentsAfterTaskCmd;
...
public class HistoryServiceImpl extends ServiceImpl implements HistoryService {
  ...
  public List<BizComment> getBizCommentsAfterTask(String businessKey){
	  return  commandExecutor.execute(new GetBizCommentsAfterTaskCmd(businessKey));
  }

@Override
public void addBizComments(String taskId, String msg,String userId,String advice) {
	 commandExecutor.execute(new AddBizCommentsCmd(taskId, msg,userId,advice));
	
}

@Override
public List<ActivityImpl> getAfterActivityCmd(String taskId) {
	
	return commandExecutor.execute(new GetAfterActivtiyCmd(taskId));
}

@Override
public void deleteBizComment(String taskId) {
	commandExecutor.execute(new DeleteBizCommentCmd(taskId));
