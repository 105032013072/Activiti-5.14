...
public class UserTaskParseHandler extends AbstractActivityBpmnParseHandler<UserTask> {
  ...
  public TaskDefinition parseTaskDefinition(BpmnParse bpmnParse, UserTask userTask, String taskDefinitionKey, ProcessDefinitionEntity processDefinition) {
    ...
    // Task listeners
    //添加默认的操作日志监听器
    ActivitiListener taskCreateListenerForComment=new ActivitiListener();
    taskCreateListenerForComment.setEvent(TaskListener.EVENTNAME_CREATE);
    taskCreateListenerForComment.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_CLASS);
    taskCreateListenerForComment.setImplementation("org.activiti.engine.impl.bpmn.listener.TaskCreateListenerForComment");
    userTask.getTaskListeners().add(taskCreateListenerForComment);
